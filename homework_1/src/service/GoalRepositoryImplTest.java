...
class GoalRepositoryImplTest {
    ...
    @BeforeEach
    void setUp() {
        goalDBMapper = mock(GoalDBMapper.class);
    ...
    void save_addsNewGoal() {
        ...
        when(goalDBMapper.toGoalDB(goal)).thenReturn(Optional.of(goalDB));
        when(goalDBMapper.toGoal(goalDB)).thenReturn(Optional.of(goal));

        goalRepository.save(goal);

        Optional<Goal> savedGoal = goalRepository.findByName("user@example.com", "Vacation");
        assertThat(savedGoal).isPresent();
        assertThat(savedGoal.get().getTargetAmount()).isEqualTo(BigDecimal.valueOf(1000));
    }
    ...
    void findByName_returnsGoalIfExists() {
        ...
        assertThat(result.get()).isEqualTo(goal);
    }
    ...
    void findAllByUser_returnsAllGoals() {
        ...
        when(goalDBMapper.toGoalDB(goal1)).thenReturn(Optional.of(goalDB1));
        when(goalDBMapper.toGoalDB(goal2)).thenReturn(Optional.of(goalDB2));
        when(goalDBMapper.toGoal(goalDB1)).thenReturn(Optional.of(goal1));
        when(goalDBMapper.toGoal(goalDB2)).thenReturn(Optional.of(goal2));
        ...
        assertThat(result).containsExactlyInAnyOrder(goal1, goal2);
    }
    ...
    void updateProgress_updatesGoal() {
        ...
        when(goalDBMapper.toGoalDB(updatedGoal)).thenReturn(Optional.of(updatedGoalDB));
        when(goalDBMapper.toGoal(updatedGoalDB)).thenReturn(Optional.of(updatedGoal)); // Добавляем настройку для updatedGoalDB

        goalRepository.updateProgress("user@example.com", "Vacation", updatedGoal);

        Optional<Goal> result = goalRepository.findByName("user@example.com", "Vacation");
        assertThat(result).isPresent();
        assertThat(result.get().getSavedAmount()).isEqualTo(BigDecimal.valueOf(500));
    }
